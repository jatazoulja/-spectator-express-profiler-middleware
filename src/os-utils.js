...
// Hard Disk Drive
exports.harddrive = async function () {
  return new Promise((res, rej) => {
    require("child_process").exec("df -k", function (error, stdout, stderr) {
      ...
      var lines = stdout.split("\n");
      let mainLine = lines[1];
      if (process.platform === "win32") {
        mainLine = mainLine.replace("Program Files", "ProgramFiles");
      }
      var str_disk_info = mainLine.replace(/[\s\n\r]+/g, ",");
      ...
      percentage = disk_info[4];
      res({ total, free, used, percentage });
    });
  });
};
...
exports.cpuFree = async function () {
  return await getCPUUsage(true);
};

exports.cpuUsage = async function () {
  return await getCPUUsage(false);
};

async function getCPUUsage(free) {
